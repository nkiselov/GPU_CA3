function makeNeuron() {
    ...
    return {
        ...
        step: dt => {
            ...
            //Voltage
            V_s += dt * ((I_exc_s + I_comp_s)/ 0.02 + (I_L_s + I_Na_s + I_K_s + I_KHT_s + I_CaBK_s + I_Ca_s) )
            V_p += dt * ((I_exc_p + I_comp_p)/ 0.02 + (I_L_p + I_A_p + I_CaBK_p + I_Ca_p))
            V_d += dt  * ((I_exc_d + I_comp_d)/ 0.06 + (I_L_d + I_A_d))
            ...
            // console.log([m_s,h_s,n_s,w_s,Ca_s,c1_s,r_s,a_p,b_p,Ca_p,c1_p,r_p,a_d,b_d])
        }
    }
}